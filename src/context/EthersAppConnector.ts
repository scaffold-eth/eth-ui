...
export class EthersAppConnector extends AbstractConnector {
  ...
  public async activate(): Promise<ConnectorUpdate> {
    ...
    if (this.web3Modal) {
      ...
      this.rawProvider = await this.web3Modal.connect();
      this.ethersProvider = new Web3Provider(this.rawProvider);
    }

    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
    const account: string = this.rawProvider?.selectedAddress ?? (await this.getAccount());
    return { provider: this.ethersProvider, account };
  }
  ...
  public deactivate(): void {
    ...
    this.rawProvider?.disconnect();

    // use close function if exists
    this.rawProvider?.close();
